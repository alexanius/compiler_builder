...
then
    ...
    then
        ...
        e "cmake -G \"$LLVM_BUILD_SYSTEM\" -DCMAKE_C_COMPILER=\"$HOST_CC\" -DCMAKE_CXX_COMPILER=\"$HOST_CPP\" -DCMAKE_INSTALL_PREFIX=\"$LLVM_INSTALL_DIR\" -DLLVM_USE_LINKER=gold -DCMAKE_BUILD_TYPE=$LLVM_BUILD_TYPE -DCMAKE_C_FLAGS=\"$LLVM_HOST_CC_OPT_FLAGS\" -DCMAKE_CXX_FLAGS=\"$LLVM_HOST_CPP_OPT_FLAGS\" -DLLVM_ENABLE_THREADS=$LLVM_ENABLE_THREADS -DLLVM_PARALLEL_COMPILE_JOBS=$BUILD_THREADS_NUMBER -DLLVM_PARALLEL_LINK_JOBS=$BUILD_THREADS_NUMBER -DLLVM_TARGETS_TO_BUILD=\"$LLVM_TARGET\" -DCMAKE_AR=\"$HOST_AR\" -DCMAKE_NM=\"$HOST_NM\" -DCMAKE_OBJCOPY=\"$HOST_OBJCOPY\" -DCMAKE_OBJDUMP=\"$HOST_OBJDUMP\" -DCMAKE_RANLIB=\"$HOST_RANLIB\" -DCMAKE_STRIP=\"$HOST_STRIP\" -DLLVM_BINUTILS_INCDIR=\"$HOST_BINUTILS_INCLUDE\" \"$LLVM_UNPACK_SRC_DIR\""
    else
